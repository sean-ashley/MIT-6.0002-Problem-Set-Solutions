...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        #create tiles in grid as dict with dirt value
        i = 0
        while i <= self.width-1:
            for k in range(0,self.height):
                self.tiles[(i,k)] = self.dirt_amount
            i+=1

    def pos_to_tile(self,pos):
        """
        Gives the tile in which the position exists
        """
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.speed = speed
        self.capacity = capacity
        #initializing at a random positon and direction
        robot_position =  room.get_random_position()
        robot_direction = random.random() * 360
    ...
    def set_random_direction(self):
        self.robot_direction = random.random() * 360
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        return self.pos_to_tile(pos) in self.furniture_tiles
    ...
    def is_position_valid(self, pos):
        ...
        """
        return 0 <= pos.get_x() <= self.width and 0 <= pos.get_y() <= self.height and self.is_position_furnished(pos) is False
    ...
    def get_random_position(self):
        ...
        """
        random_pos = Position(random.random()*self.width,random.random()*self.height) 
        while self.is_position_furnished(random_pos) is True:
            random_pos = Position(random.random()*self.width,random.random()*self.height)
            continue
        return random_pos
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        #if the tile has dirt on it and the robot can physically get there, stay there clean it.
        if self.room.is_position_valid(position) and self.room.tiles[self.room.pos_to_tile(position)] > 0:
            self.room.clean_tile_at_position(position,self.capacity)
        #other wise, change direction 
        else:
            self.set_random_direction()
            direction = self.get_robot_direction()
            position = self.get_robot_position().get_new_position(direction,self.speed)
            #if direction is valid, move there 
            if self.room.is_position_valid(position):
                self.set_robot_position(position)
                #if direction has dirt, clean it
                if self.room.tiles[self.room.pos_to_tile(position)] > 0:
                    self.room.clean_tile_at_position(position,self.capacity)
            

        

#Run this code to visualize the robot moving!
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        #if the tile has dirt on it and the robot can physically get there and its not faulty, stay there clean it.
        faulty = self.gets_faulty()
        position = self.get_robot_position()
        direction = self.get_robot_direction()
        if faulty:
            self.set_random_direction()
        elif self.room.is_position_valid(position) and self.room.tiles[self.room.pos_to_tile(position)] > 0:
            self.room.clean_tile_at_position(position,self.capacity)
        #other wise, change direction 
        else:
            self.set_random_direction()
            ...
            if self.room.is_position_valid(position):
                self.set_robot_position(position)
                #if direction has dirt and the robot isnt faulty, clean the dirt
                if self.room.tiles[self.room.pos_to_tile(position)] > 0 :
                    self.room.clean_tile_at_position(position,self.capacity)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        #initializing varlables
        time_steps = 0
        coverage = 0
        room = EmptyRoom(width,height,dirt_amount)
        robot_list = []
        #make a robot object for each robot in th list
        for robot in range(num_robots):
                robot_list.append(robot_type(room,speed,capacity))
        #while the min amount of coverage isnt met , continue cleaning
        while coverage < min_coverage:
            time_steps += 1 
            #moves however many robot are in the robot list
            for robot in robot_list:
                #moves the robot and cleans
                robot.update_position_and_clean()
                #updates coverage
                coverage = float(room.get_num_cleaned_tiles())/room.get_num_tiles()
        #creates a list of all the times of the trials
        time_step_list.append(time_steps)
    #returns the mean of the time step list
    return sum(time_step_list)/len(time_step_list)



# print ('avg time steps faulty: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1.0, 50, FaultyRobot)))
# print ('avg time steps standard: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1.0, 50, StandardRobot)))
# print ('avg time steps f: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, FaultyRobot)))
# print ('avg time steps s: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, StandardRobot)))
# print ('avg time stepsf: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, FaultyRobot)))
# print ('avg time stepss: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, StandardRobot)))
# print ('avg time stepsf: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, FaultyRobot)))
# print ('avg time stepss: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
# print ('avg time stepsf: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.5, 50, FaultyRobot)))
# print ('avg time stepss: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.5, 50, StandardRobot)))
