...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {} # tiles are a dictionary of (x, y) > int. tile location mapped to tile dirtyness amount



        for w in range(math.floor(width)):

            for h in range(math.floor(height)):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if (tile_x, tile_y) not in self.tiles:
            ...
            if self.tiles[(tile_x, tile_y)] - capacity < 0:
    ...
    def get_num_cleaned_tiles(self):
        ...
        for w in range(math.floor(self.width)):

            for h in range(math.floor(self.height)):
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.direction = random.random() * 360 #angle: float representing angle in degrees, 0 <= angle < 360
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        #Available tiles are total tiles in room - those that are furnished
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        current_dir = self.get_robot_direction()

        current_speed = self.speed

        new_pos = self.get_robot_position().get_new_position(current_dir, current_speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    return sum(steps_per_trial) / num_trials
